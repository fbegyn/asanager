let config = {
    ...
    alertmanagerUrl: "/api/alertmanager",
    alertmanagerServerUrl: "",
    labels: [],
};
...
async function fetchServerConfig() {
    try {
        ...
        if (data.alertmanager_url) {
            config.alertmanagerServerUrl = data.alertmanager_url;
            console.log("Alertmanager server URL loaded from server:", config.alertmanagerServerUrl);
        } else {
            console.warning("Failed to load alertmanager server URL from config")
        }
    } catch (error) {
...
function createSilence() {
    ...
    fetch(`${config.alertmanagerUrl}/api/v2/silences`, {
        ...
        "\n\nSuccess! Silence created with ID:\n" + data.silenceID;
            elements.response.textContent +=
        "You can view the silence at " + config.alertmanagerServerUrl + "/#/silences/" + data.silenceID;
